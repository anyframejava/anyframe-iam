...
package org.anyframe.iam.admin.candidatesecuredresources.service;

import org.anyframe.generic.service.GenericService;
import org.anyframe.iam.admin.domain.CandidateSecuredResources;

/**
 * An interface that provides services about CandidateSecuredResources table.
 * 
 * @author youngmin.jo
 */
public interface CandidateSecuredResourcesService extends GenericService<CandidateSecuredResources, String> {
	...
	/**
	 * check pattern if the given pattern is valid
	 * @param savePattern pattern name that want to save
	 * @param resourceType resource type that the pattern has
	 * @return if pattern name is valid, then return true boolean value. else,
	 * false.
