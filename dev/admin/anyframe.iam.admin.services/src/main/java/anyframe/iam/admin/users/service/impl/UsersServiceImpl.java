...
public class UsersServiceImpl extends GenericServiceImpl<Users, String> implements UsersService {
	...
	public Users get(String id) throws Exception {
		...
		Set<GroupsUsers> groupsUsers = user.getGroupsUserses();
		 ...
		 */
		if (groupsUsers.iterator().hasNext()) {
			Hibernate.initialize(groupsUsers.iterator().next().getGroups());
			if (groupsUsers.iterator().next().getGroups().getGroupsUserses().iterator().hasNext()) {
				Hibernate.initialize(groupsUsers.iterator().next().getGroups().getGroupsUserses().iterator().next()
						.getId().getGroupId());
			}
		}
	...
	public Users update(Users users, GroupsUsers newGroupsUsers, GroupsUsers currentGroupsUsers,
		...
		/**
		 * GROUPS_USERS 테이블의 Group / User 매핑 정보 등록
		 */
		groupsUsersDao.save(newGroupsUsers);
