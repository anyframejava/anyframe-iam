...
<html>
<head> 
<title>Search List of Production</title>
...
function fncGetProductSearchList(arg) {
	if(arg == "1")
		document.listForm.pageIndex.value = 1;
   	document.listForm.action="<c:url value='/emplistproductsearch.do'/>";
   	document.listForm.submit();		
}

function fncGetProductSearch(prodNo) {
	document.listForm.prodNo.value = prodNo;
   	document.listForm.action="<c:url value='/empgetproductsearch.do'/>";
   	document.listForm.submit();		
}
...
<form name="listForm"  method="post" action="<c:url value='/emplistproductsearch.do'/>">
<!--************************** begin of contents *****************************-->
	...
	<tr>
		...
		<td background="<c:url value='/sample/images/ct_ttl_img02.gif'/>" width="100%" style="padding-left:10px;">
			<table width="100%" border="0" cellspacing="0" cellpadding="0" style=" width : 941px; height : 20px;">
				<tr>
					<td width="93%" class="ct_ttl01">Search List of Production</td>
				</tr>
			</table>
		</td>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
	<tr>
		...
		<td align="left">
			<select name="searchAsYn" class="ct_input_g" style=" width : 92px;">
				<option value="A">All</option>
				<option value="Y" <%= ("Y".equals(asYn) ? "selected" : "")%>>Possible</option>
				<option value="N" <%= ("N".equals(asYn) ? "selected" : "")%>>Impossible</option>				
			</select>		
		</td>
			...
			<table border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td background="<c:url value='/sample/images/ct_btnbg02.gif'/>" class="ct_btn01" style="padding-top:3px;">
						<a href="javascript:fncGetProductSearchList('1');">Search</a>
					</td>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
	...
	<tr class="ct_list_pop">
		...
		<td></td>
		<td align="center"><a href="javascript:fncGetProductSearch('<%= productVO.getProdNo() %>');"><%=productVO.getProdNo() %></a></td>
		<td></td>
		<td align="left"><%= productVO.getProdName() %></td>
		<td></td>
		<td align="right"><%= productVO.getSellAmount() %></td>
		<td></td>
		<td align="center"><%= "Y".equals(productVO.getAsYn()) ? "Possible" : "Impossible" %></td>
		<td></td>
		<td align="center"><%= EmpUtil.toDateStr(productVO.getRegDate().toString()) %></td>
	</tr>
	<tr>
