 ...
 * 
 */
public class ReloadableRestrictedTimesFilterInvocationSecurityMetadataSource implements FilterInvocationSecurityMetadataSource,
	...
	public void reloadRestrictedTimes() throws BaseException {
		try {
			...
			logger.error(getMessageSource().getMessage("[SecuredObject Service] SecuredObject Service [" + "Reload RestrictedTimes" + "]: Fail to get SecuredObject Data from Database.",
	...
	public List<ConfigAttribute> lookupRoleOrUrlInCandidateListCheck(List compareCandidateList, String url,
		...
		while (iter.hasNext()) {
			...
			if ((dailyExceed && dailyExceedMatched)
				...
				foundCad = (isRoleCheck ? (List<ConfigAttribute>) map.get("restrictedRoleIds") : checkUrlMatching(
						map, url));
				if (foundCad != null) {
